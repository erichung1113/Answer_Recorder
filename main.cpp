...
int main() {
    ...
    while (true) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (Data[questions[i]].correct != "") {
                for (int j = 0; j < options.size(); j++) {
                    if (options[j] == Data[questions[i]].correct)
                        ans[i] = j;
                }
            } else {
        ...
        getline(cin, trash);
        cin.ignore();
        if (trash[0] == 'C') getline(cin, trash), cin.ignore();
        getline(cin, trash);
